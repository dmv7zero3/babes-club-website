...
Key features:
- Gracefully handles users with no orders (returns empty list)
- Handles users without Stripe ID (no orders until first purchase)
...
def _get_user_id(event: Dict[str, Any]) -> Optional[str]:
    ...
    - Custom authorizer with userId in context
    - Cognito authorizer with sub claim
    - JWT authorizer with claims
    """
    request_context = event.get("requestContext") or {}
    authorizer = request_context.get("authorizer") or {}
    
    # Log the authorizer context for debugging
    logger.debug("Authorizer context: %s", json.dumps(authorizer, default=str))
    ...
    user_id = (
        ...
        (authorizer.get("claims") or {}).get("sub") or  # JWT claims
        (authorizer.get("jwt") or {}).get("claims", {}).get("sub")  # HTTP API JWT
    )
...
def lambda_handler(event: Dict[str, Any], _context: Any) -> Dict[str, Any]:
    ...
    Returns:
        {
            "orders": [...],
            "nextCursor": "..." or null,
            "hasOrders": true/false,  # Added for frontend convenience
            "message": "..." # Optional message for empty state
        }
    """
    # Log the request for debugging
    logger.info(
        "List orders request: path=%s method=%s",
        event.get("path"),
    ...
    if not user_id:
        ...
        # Log the full request context for debugging (excluding sensitive data)
        logger.debug(
            "Request context: %s",
    ...
    logger.info("Processing orders request for user: %s", user_id)
    ...
    # Build response with helpful metadata for frontend
    ...
    if not has_orders and not cursor:
        result["message"] = "You haven't placed any orders yet. Your order history will appear here after your first purchase."
    ...
    return _json_response(result, cors_origin=cors_origin)
