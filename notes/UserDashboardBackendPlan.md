...
- **Secrets & Config**: Signing keys, Stripe secret injected as encrypted Lambda environment variables during deployment and rotated manually when required.
...
6. **`stripe_webhook_handler`**: Process Stripe events, update `OrderSnapshots`, mark `lastSyncedAt`.
7. **`stripe_order_sync`** (nightly CloudWatch event): Reconcile Stripe history to fill gaps.
...
1. Capture Babes Club JWT signing secret and Stripe credentials, encrypt, and wire into deploy pipeline for Lambda env vars.
2. Draft DynamoDB table definitions (`UserProfiles`, `OrderSnapshots`) with throughput settings and required GSIs.
...
7. Set up CloudWatch metrics, structured logging, and alarms for each Lambda; ensure audit events capture profile updates.
8. Expose internal sync endpoints with hardened auth (shared internal token) and document usage.
9. Build React dashboard components listed above, wiring them to the API via context hooks and adding optimistic refresh behavior.
