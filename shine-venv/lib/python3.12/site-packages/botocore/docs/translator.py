...
class BotoHTML5Translator(SphinxHTML5Translator):
    ...
    def is_implicit_playfair(self, node):
        """Determines if a node is an implicit playfair.

        An implicit playfair is represented by a paragraph node whose only
        child is a strong node with text that isnt in `IGNORE_IMPLICIT_playfairS`.
        """
        return (
            len(node) == 1
            ...
            and node[0][0].astext() not in self.IGNORE_IMPLICIT_playfairS
        )

    def visit_paragraph(self, node):
        """Visit a paragraph HTML element.

        Replaces implicit playfairs with an h3 tag and defers to default
        behavior for normal paragraph elements.
        """
        if self.is_implicit_playfair(node):
            text = node[0][0]
